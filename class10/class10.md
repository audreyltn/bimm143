...
The variable `winpercent()` shows the percentage of people who prefer
this candy over another randomly chosen candy from the dataset. Higher
...
The `skim()` function in the **skimr** package that can help give a
...
No, it’s not symmetrical. There are more values (higher frequency) on
the lower side.
...
This difference is statisically different because the p-value is \<
0.01.
...
I like using the `order()` function in R better, because you don’t have
to download the **dplyr** package. I also don’t like typing the %\>%
every time.
...
Reese Miniatures has a `winpercent` value \> 80 and is relative cheap
with a `pricepercent` of slightly above 0.25.
...
Use the **ggrepel** package and the function
`ggrepel::geom_text_repel()` to label the plot with nonoverlapping candy
names. We can also add a title and subtitle.
...
If you want to see mroe candy labels, you can change the `max.overlaps`
...
Fruity and pluribus are picked up the strongest in the positive
direction. This makes sense, because there’s a lot of variation in
fruits and pluribus (multiple candies).
